...
router.get('/:org/:version', jsonParser, (req, res) => {
  ...
  if (!versionIsCompatible(version, orgDetails)) {
    ...
    Promise.all([
      ...
      getDependency(
        `${orgDetails.label}-github`,
        orgDetails.githubUrl,
        version
      ).then((path) =>
        getDirectories(`${path}/${orgDetails.componentDir}`)
          .map((componentName) =>
            fs
              .readFileAsync(
              ...
              .then((componentInfo) =>
                componentInfo.type === 'layout' ? [] : componentInfo.examples
              )
              .catch(() => [])
              .map((example) => ({
                componentName: getComponentSignature(
                  orgDetails.code,
                  componentName
                ),
                exampleName: example.name,
                exampleId: ensureUniqueName(
                  `${componentName}-${example.name}`.replace(/\s/g, '-')
                ),
                input: example.data,
              }))
          )
      ...
      .spread((configuredNunjucks, examples) =>
