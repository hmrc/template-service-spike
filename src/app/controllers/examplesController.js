...
const getLatestExamples = (name) =>
  getLatestSha(name).then((sha) =>
    getDependency(name, `https://github.com/${name}/tarball/${sha}`, sha)
  )
...
router.get('/:org', (req, res) => {
  ...
  getLatestExamples(name)
    .then((examplePath) =>
      getDirectories(`${examplePath}/${componentRootPath}`)
    )
    .then((dirs) => {
...
router.get('/:org/:component', (req, res) => {
  ...
  getLatestExamples(name)
    .then((dependencyPath) =>
      getSubDependencies(dependencyPath, dependencies).then(
        (subdependecyPaths) => ({
          dependencyPath,
          subdependecyPaths: [
            ...subdependecyPaths,
    ...
    .then((paths) => {
      ...
      return getDirectories(componentPath).map((example) =>
        getDataFromFile(
          `${componentPath}/${example}/index.njk`,
          paths.subdependecyPaths
        )
          .catch((err) => {
