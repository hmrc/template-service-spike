...
const dirExistsAndNotEmpty = (path) =>
  fs
    .readdirAsync(path)
...
const getComponentIdentifier = (org, component) =>
  component
    .replace(/([A-Z])/g, (g) => `-${g[0].toLowerCase()}`)
    .replace('govuk-', '')
    .replace('hmrc-', '')
const getComponentSignature = (org, component) =>
  org +
  component
...
const getDataFromFile = (file, paths) =>
...
const getNpmDependency = (dependency, version) =>
  getDependency(
    dependency,
...
const getSubDependencies = (dependencyPath, dependencies) =>
  Promise.all(
    dependencies.map((dependency) =>
      loadJsonFile(`${dependencyPath}/package.json`).then((packageContents) => {
...
const joinWithCurrentUrl = (req, path) =>
  `${req.originalUrl.replace(/\/+$/, '')}/${path}`

const versionIsCompatible = (version, org) =>
  parseFloat(version) >= org.minimumSupported

const getConfiguredNunjucksForOrganisation = (org, version) =>
  getNpmDependency(org.label, version)
    .then((path) =>
      getSubDependencies(
        path,
        org.dependencies || []
